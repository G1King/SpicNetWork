...
@end
@implementation CustomUpdateView
...
- (instancetype)initWithimage:(UIImage *)image message:(NSString*)message sureBtn:(NSString *)sureTitle cancleBtn:(NSString *)cancleTitle
{
    if (self == [super init]) {
        ...
        if (image) {
            ...
            self.imageView = [[UIImageView alloc]initWithFrame:CGRectMake(0, -5, AlertW, 55)];
            self.imageView.image= image;
            [self.alertView addSubview:self.imageView];
            
        }
        UILabel *label = [[UILabel alloc]initWithFrame:CGRectMake(5, 5, 100, 15)];
        label.text = @"更新内容";
        ...
        self.sLineView.backgroundColor = [UIColor colorWithWhite:0.8 alpha:0.6];
        [self.alertView addSubview:self.sLineView];
